...
namespace MyApiNight.WebUI.Areas.Admin.Controllers
{
	...
	public class WriterController : Controller
	{
		...
		public async Task<IActionResult> WriterList()
		{
			...
			var responseMessage = await client.GetAsync("https://localhost:7123/api/Writer");
		...
		public async Task<IActionResult> CreateWriter(CreateWriterDto createWriterDto)
		{
			...
			var reponseMessage = await client.PostAsync("https://localhost:7123/api/Writer", stringContent);
		...
		{
			var client = _httpClientFactory.CreateClient();
			var responseMessage = await client.DeleteAsync("https://localhost:7123/api/Writer?id=" + id);
			if (responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("WriterList");
			}
		...
		{
			var client = _httpClientFactory.CreateClient();
			var responseMessage = await client.GetAsync("https://localhost:7123/api/Writer/GetWriter?id=" + id);
			if (responseMessage.IsSuccessStatusCode)
			{
				var jsonData = await responseMessage.Content.ReadAsStringAsync();
		...
		public async Task<IActionResult> UpdateWriter(UpdateWriterDto updateWriterDto)
		{
			...
			StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
			var responseMessage = await client.PutAsync("https://localhost:7123/api/Writer", stringContent);
			if (responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("WriterList");
			}
